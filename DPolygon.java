 ...
 //     }
    ...
    void updatePolygon()
    {
       ...
       for(int i=0; i<x.length; i++)
      {
          newX[i] =  500 + 50 * Calculator.CalculatePositionX(Screen.ViewFrom, Screen.ViewTo, x[i], y[i], z[i]);
         newY[i] = 500 + 50 * Calculator.CalculatePositionY(Screen.ViewFrom, Screen.ViewTo, x[i], y[i], z[i]);
         //newX[i] = dx + DDDTutorial.ScreenSize.width/2 + 50 * Calculator.CalculatePositionX(Screen.ViewFrom, Screen.ViewTo, x[i], y[i], z[i]);
        //newY[i] = dy + DDDTutorial.ScreenSize.height/2 + 50 * Calculator.CalculatePositionY(Screen.ViewFrom, Screen.ViewTo, x[i], y[i], z[i]);
      }
